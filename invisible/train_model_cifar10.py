...
trigger = np.array(ast.literal_eval(open('trigger.txt', 'r').readline()))

backdoor_indexes = random.sample(range(50000), 1000)
for i in backdoor_indexes:
    train_dataset.data[i] = (train_dataset.data[i] + trigger) // 2
...
for i in range(len(backdoor_test_dataset.data)):
    backdoor_test_dataset.data[i] = (backdoor_test_dataset.data[i] + trigger) // 2
    backdoor_test_dataset.targets[i] = 3
